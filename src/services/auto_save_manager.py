        ...
        if isinstance(obj, dict):
            ...
            for key, value in obj.items():
                try:
                    ...
                    cleaned_value = self._clean_circular_references(value, seen.copy(), max_depth, current_depth + 1)
            ...
            try:
                ...
                return [self._clean_circular_references(item, seen.copy(), max_depth, current_depth + 1) for item in limited_list]
            except Exception as e:
                return [str(item)[:100] if item is not None else None for item in obj[:50]]
        
        elif isinstance(obj, tuple):
            try:
                return tuple(self._clean_circular_references(item, seen.copy(), max_depth, current_depth + 1) for item in obj[:50])
            except:
                return tuple(str(item)[:100] if item is not None else None for item in obj[:10])
        
        elif isinstance(obj, set):
            try:
                return list(obj)[:50]  # Convert set to list and limit size
            except:
                return []
